...
if __name__ == '__main__':
    ...
    for j in range(0, nc):
        ...
        print(X.shape)
        if train:
            # name = 'F:/LargeDataFile/ILSVRC/miniImgNet/miniImgNet_ViT/train_ViT-B32_224_ImageNetNorm/%s.mat' % (names[j])
            name = 'F:/LargeDataFile/ILSVRC/miniImgNet/miniImgNet_ViT/train_ViT-L14_224_ImageNetNorm/%s.mat' % (
            names[j])
            savemat(name, {'feature': X.float().cpu().numpy()})

        else:
            # name = 'F:/LargeDataFile/ILSVRC/miniImgNet/miniImgNet_ViT/val_ViT-B32_224_ImageNetNorm/%s.mat' % (names[j])
            name = 'F:/LargeDataFile/ILSVRC/miniImgNet/miniImgNet_ViT/val_ViT-L14_224_ImageNetNorm/%s.mat' % (names[j])
